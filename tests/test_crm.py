...
@pytest.mark.asyncio
async def test_affinity_api_get_field_value_changes_raises_error_no_field_id():
    with pytest.raises(ValueError) as excinfo:
        affinity_client = AffinityApi("test")
        await affinity_client.get_field_value_changes(params={})

    assert excinfo.type is ValueError
    assert excinfo.value.args[0] == "Field ID must be specified"
